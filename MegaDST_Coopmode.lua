...
function OnLoad()
	...
	DST:addSubMenu("CO-OP ~ AI Version", "Coop")
...
function KillPos(i)
	...
	if DST["positiveto"..i] == true and allies[i].type == "AIHeroClient" then
			...
			--print("ranN: "..ranN..", MenuTable #: "..MenuTable[ranN])
		...
		if gameState == true then
			...
			if MenuTable[ranN] > 1 and DST.goodjob.compare then
				if ranName > 50 then
					SendChat(SpecificBoost[ math.random( #SpecificBoost ) ]..string.lower(string.sub(allies[i].name, 1,math.random(5,10)))..", "..LosingQuotes[ math.random( #LosingQuotes ) ])
				else
					SendChat(SpecificBoost[ math.random( #SpecificBoost ) ]..string.lower(string.sub(playchampname[i], 1,math.random(4,7)))..", "..LosingQuotes[ math.random( #LosingQuotes ) ])
				end
			elseif MenuTable[ranN] == 1 and DST.goodjob.personal then
...
function OnTick()
	...
	if not DST.goodjob.compare and compareOff then
		compareOff = false
		for i = 1, #MenuTable do
			if MenuTable[i] == 1 then
	...
	if DST.goodjob.personal and not personalOff then
		personalOff = true
	...
	if not DST.goodjob.personal and personalOff then
		personalOff = false
		for i = 1, #MenuTable do
			if MenuTable[i] == 2 then
				table.remove(MenuTable, 2)
				table.remove(MenuTable, 2)
			end
		end
		table.remove(MenuTable, 2)
	...
	if DST.goodjob.simple and not simpleOff then
		simpleOff = true
		table.insert(MenuTable, 3)
		table.insert(MenuTable, 3)
		table.insert(MenuTable, 3)
		table.insert(MenuTable, 3)
	end
	if not DST.goodjob.simple and simpleOff then
		simpleOff = false
		for i = 1, #MenuTable do
			if MenuTable[i] == 3 then
				table.remove(MenuTable, 3)
				table.remove(MenuTable, 3)
			end
		end
	end
--Send(myHero.name.." has "..myHero.minionScore.."kills")
...
if awards ~= nil and allies[1].wardPlaced ~= nilthen
...
if akwards ~= nil and allies[i].wardKill ~= nilthen
...
if os.clock() > deathDelay and myHero.dead and DST.Sorry then
	...
	deathDelay = os.clock() + (DST.SDelay*60) -- DELAY METHOD VERY IMPORTANT
end
